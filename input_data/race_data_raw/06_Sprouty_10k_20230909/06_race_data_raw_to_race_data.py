...
path_to_10k_raw = "input_data/race_data_raw/06_Sprouty_10k_20230909/10k_raw.csv"
path_to_5k_raw = "input_data/race_data_raw/06_Sprouty_10k_20230909/5k_raw.csv"


def _parse_sprouty_net_time(x: str) -> str:
    """
...
def _process_path(path: str, which_race: str):
    ...
    with open(path, newline="") as csvfile:
        ...
        for row in reader:
            ...
            net = _parse_sprouty_net_time(row[7])
            d["First Name"].append(first)
            d["Last Name"].append(last)
            d["Net Time"].append(net)
    df = pd.DataFrame(d)
    logger.info(df)
    df.to_csv(f"input_data/race_data/06_Sprouty_10k_20230909/{which_race}.csv")
