...
```{shell}
python
input_data/race_data_raw/05_HurricaneHill_10k_20230826/05_race_data_raw_to_race_data.py
```
"""
...
path_to_10k_raw = "input_data/race_data_raw/05_HurricaneHill_10k_20230826/10k_raw.csv"
path_to_5k_raw = "input_data/race_data_raw/05_HurricaneHill_10k_20230826/5k_raw.csv"


def _parse_hurricane_hill_net_time(x: str) -> str:
    """
...
def _process_path(path: str, which_race: str):
    ...
    with open(path, newline="") as csvfile:
        ...
        for row in reader:
            ...
            first = row[1]
            last = row[2]
            net = _parse_hurricane_hill_net_time(row[6])
            d["First Name"].append(first)
            d["Last Name"].append(last)
            d["Net Time"].append(net)
    df = pd.DataFrame(d)
    logger.info(df)
    df.to_csv(f"input_data/race_data/05_HurricaneHill_10k_20230826/{which_race}.csv")
